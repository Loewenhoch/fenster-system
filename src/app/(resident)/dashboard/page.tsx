...
export default async function DashboardPage() {
  ...
  const latestOrder = apartment.orders[0];
  const hasOrder = latestOrder?.status === "CONFIRMED" || latestOrder?.status === "DRAFT";
  const isConfirmed = latestOrder?.status === "CONFIRMED";

  const orderableWindows = windows.filter((w) => w.isOrderable);
  const windowsWithInterest = windows.filter(
        ...
        <CardContent>
          <div className="grid gap-4 sm:grid-cols-3">
            <div className="rounded-lg bg-secondary p-4">
              ...
              <p className="text-sm text-muted-foreground">{building.street}</p>
            </div>
            <div className="rounded-lg bg-secondary p-4">
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
